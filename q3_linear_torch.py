...
class Linear(DQN):
    ...
    def initialize_models(self):
        ...
        input_size = n_channels * self.config.state_history * img_height * img_width
    ...
    def calc_loss(
        ...
        q_samp = (
            rewards + (~done_mask) * gamma * torch.max(target_q_values, dim=1).values
        )
        q_sa = q_values[range(len(q_values)), actions.type(torch.LongTensor)]
        return F.mse_loss(q_samp, q_sa)
